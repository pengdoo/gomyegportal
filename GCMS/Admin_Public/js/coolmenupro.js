window.CMenus=[];
var BLANK_IMAGE="Admin_Public/Images/Menu/b.gif";
function bw_check(){
...
function COOLjsMenuPRO(name,items){
	...
	this.u="http://www.huntfox.net"+this.cc+this.z;
	this.items=[];
...
function CMenuLevel(menu,par){
	...
	this.vis=function(s){
		...
		if(!s)
			this.a=null;
		if(this.v!=ss&&this.menu.onlevelshow)
...
function CMenuItem(menu,par,item,format){
	...
	this.par=par;
	this.code=item.code;
	this.ocode=item.ocode||item.code;this.targ=und(item.target)?"":'target="'+item.target+'"';
	this.url=und(item.url)?"javascript:none()":item.url;this.fmt=format;
	this.menu=menu;this.bw=menu.bw;this.cd=[];this.divs=[];
	this.index=menu.items.length;menu.items=menu.items.concat([this]);
	this.pindex=par.cd.length;par.cd=par.cd.concat([this]);
		...
		case "c":
			if(window.CMenus[m].items[node_index].hc())
				...
				for(var i=0;i<window.CMenus[m].root.cd.length;i++)
					...
					if (window.CMenus[m].onclick)
						window.CMenus[m].onclick(window.CMenus[m].items[node_index]);
			break;
		case "t":
...
function CMOnLoad(){
	...
	if(bw.ns4||bw.opera){
		...
		if (bw.opera&&!window.operaResizeTimer)
			resizeHandler();
		if(bw.ns4)
			window.onresize=resizeHandler;
			}
...
if(new bw_check().ns4)
	window.onresize=resizeHandlerelse;
