...
var sMenu1="<TABLE border=0 cellpadding=0 cellspacing=0 class=Menu width=150><tr><td width=18 valign=bottom align=center style='background:url("+ config.SysImage +"/.gif);background-position:bottom;'><\/td><td width=132 class=RightBg><TABLE border=0 cellpadding=0 cellspacing=0>";
...
function getFormatMenuRow(menu, html, image){
	var s_Disabled = "";
	if (!SeaskyEditor.document.queryCommandEnabled(menu)){
		s_Disabled = "disabled";
	}
...
function showContextMenu(event){
	...
	if (isControlSelected("IMG")){
		sMenu += getMenuRow("", "ShowDialog('Dialog/EditorImg.html', 380, 315, true)", "img.gif", "图片属性...");
		sMenu += sMenuHr;
	...
	oPopupMenu.show(lefter, topper, width, height, SeaskyEditor.document.body);
	return false;
...
function showToolMenu(menu){
	...
	case "paragraph":	// 段落菜单
		...
		break;
	case "edit":		// 编辑菜单
		sMenu += getFormatMenuRow("Undo", "撤消", "undo.gif");
		sMenu += getFormatMenuRow("Redo", "恢复", "redo.gif");
		sMenu += sMenuHr;
	...
	case "object":		// 对象效果菜单
		...
		break;
	case "component":	// 组件菜单
		sMenu += getMenuRow("", "ShowDialog('dialog/EditorImg.html', 380, 315, true)", "img.gif", "插入或修改图片");
		sMenu += getMenuRow("", "ShowDialog('dialog/EditorFlash.html', 350, 200, true)", "flash.gif", "插入Flash动画");
		sMenu += getMenuRow("", "ShowDialog('dialog/EditorMedia.html', 350, 200, true)", "media.gif", "插入自动播放媒体");
		sMenu += getMenuRow("", "ShowDialog('dialog/EditorFile.html', 350, 150, true)", "file.gif", "插入其他文件");
		sMenu += sMenuHr;
		sMenu += getMenuRow("", "ShowDialog('dialog/fieldset.htm', 350, 170, true)", "fieldset.gif", "插入或修改栏目框");
		sMenu += getMenuRow("", "ShowDialog('dialog/EditorIframe.html', 400, 260, true)", "iframe.gif", "插入或修改网页帧");
	...
	case "file":		// 文件视图菜单
		...
		sMenu += getMenuRow("", "ShowDialog('dialog/about.htm','400','220')", "about.gif", "关于SeaskyEditor");
		height = 208;
		break;
	case "table":		// 表格菜单
