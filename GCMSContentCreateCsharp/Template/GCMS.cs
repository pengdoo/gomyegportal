...
namespace GCMSContentCreate
{
    ...
    public class GCMS
    {
        ...
        public Collection Channels()
        {
            ...
            sql = "SELECT Top " + ListTop + " Content_ID,Url,OrderNum,* FROM " + FieldsName + " WHERE ( Content_ID in (select Content_ID from Content_Commend WHERE  TypeTree_ID = " + ChannelID.ToString() + ") or TypeTree_ID =" + ChannelID.ToString() + " ) " + strListLastID + " and status = 4 "+GetUserWhere()+" order by " + Order;
            myReader = Tools.DoSqlReader(sql);
            while (myReader.Read())
            {
        ...
        public Collection Top(int intTop)
        {
            ...
            sql = "SELECT Top " + intTop.ToString() + "  " + FieldsName + ".Content_ID ,Url ," + FieldsName + ".TypeTree_ID,* FROM " + FieldsName + " WHERE (TypeTree_ID in (" + GetChannelID + ") or " + FieldsName + ".content_ID in (select distinct Content_ID from Content_Commend where TypeTree_ID in (" + GetChannelID + "))) " + isNews + " and Status = 4 "+ GetUserWhere()+ " order by " + Orderby;
            myReader = Tools.DoSqlReader(sql);
